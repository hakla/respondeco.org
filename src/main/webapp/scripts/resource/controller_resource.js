...
respondecoApp.controller('ResourceController', function($scope, $location, $routeParams, Resource, Account, Organization) {
  ...
  $scope.search = function() {
    $scope.resourceSearch.tags = $.map($scope.searchTags, function(tag) {
      return tag.name
    }).join(","); //create comma separated list
  ...
  $scope.update = function(id) {
    Resource.get({
      ...
      $scope.selectedTags = $.map($scope.resource.resourceTags, function(tag) {
        return {
          name: tag
        }
